# -*- coding: utf-8 -*-
"""
Created on Wed Jul 10 15:35:27 2024
...
from ckn_training import FCN_CKN
import torch
...
def get_model_identifier(loss, transformation, target, otherClass, split_isoforms, special_id='') :
    return f'{"splitIsoforms" if split_isoforms else "nonprior"}{special_id}_{loss}_{transformation}_{target}{"_otherVector" if otherClass else "_noOtherVector"}'

def predict_model(data_path, metadata_path=None, loss='crossentropy', transformation='log2', target='both', include_other_class=False, split_isoforms=False, special_id='') :
    ...
    if include_other_class or target == 'both' :
        ...
        model = FCN_CKN(output_size=first_output_size, columns=dataset.columns.tolist())
...
if __name__ == '__main__':
    ...
    predict_model('data/limma_data.tsv', loss=loss, transformation=transformation, target=target, include_other_class=include_other_class)
